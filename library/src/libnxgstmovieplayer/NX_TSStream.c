...
dump_collection (GstStreamCollection * collection, MpegTsSt *handle)
{
	...
	NXGLOGI("############# START OF dump_collection ##############");
	...
	for (i = 0; i < gst_stream_collection_get_size (collection); i++) {
		...
		for (int i=0; i<handle->media_info->n_program; i++)
		{
			if (handle->media_info->program_number[i] == handle->media_info->current_program) {
				cur_pro_idx = i;
				NXGLOGI("Found matched program number! program_number:%d, idx:%d",
						handle->media_info->program_number[i], i);
			}
		}
		...
		else if (stype & GST_STREAM_TYPE_VIDEO)
		{
			...
			NXGLOGI("## video_mpegversion(%d), type(%d), stream_id(%s), n_video(%d)",
					video_mpegversion, handle->media_info->ProgramInfo[cur_pro_idx].VideoInfo[0].type,
					stream_id, handle->media_info->ProgramInfo[cur_pro_idx].n_video);
		}
	...
	NXGLOGI("############# END OF dump_collection ##############");
	MediaInfoToStr(handle->media_info, "");
}
	...
	switch (GST_MESSAGE_TYPE (message)) {
		...
		case GST_MESSAGE_ELEMENT:
		{
			...
			if ((section = gst_message_parse_mpegts_section(message))) {
				...
				NXGLOGI("Got section: PID:0x%04x type:%s (table_id 0x%02x (%s)) at offset %"
				...
				{
					NXGLOGI("   subtable_extension:0x%04x, version_number:0x%02x",
							section->subtable_extension, section->version_number);
					NXGLOGI("   section_number:0x%02x last_section_number:0x%02x crc:0x%08x",
...
on_decodebin_pad_added(GstElement *element, GstPad *pad, gpointer data)
{
	...
	if (g_str_has_prefix(mime_type, "video/"))
	{
		...
		// There is no video type, mpeg version
		//int32_t v_idx = handle->media_info->ProgramInfo[cur_pro_idx].n_video;
